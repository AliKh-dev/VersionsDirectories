...
--71560CCAI152EJ3-AlterSPWithLog.DDL
...
BEGIN  
  ...
  ------------------------------------------------------------------------------------------------------  
     ...
     truncate table [InquiryReportCalculationPipelineLog]


  	insert into [InquiryReportCalculationPipelineLog]([Guid],Step,[State],[Description],CreationDate)
    values(NEWID(),'Start','Started',N'شروع محاسبات ',GetDate())
  ...
  truncate table ETL_TMP_InquiryReportData  
    ...
    insert into [InquiryReportCalculationPipelineLog]([Guid],Step,[State],[Description],CreationDate)
    values(NEWID(),'Drop Temp Tables','done',N'اتمام دراپ جداول تمپ',GetDate())
...
select @LoanStatusTypeLoanPaymentStatusId=cast(value as bigint) from Vw_Config where [key]=N'com:LoanStatusTypeLoanPaymentStatusId';  
   ...
   insert into [InquiryReportCalculationPipelineLog]([Guid],Step,[State],[Description],CreationDate)
    values(NEWID(),'Set Variables','done',N'مقداردهی متغیرها',GetDate());
 ...
 select MainCustomerNumber,MainCustomerId from Vw_LCRemained  where  cast(CalculateDate as date)=@CalculateDate  
   ...
   select * into #CustomerListForCalc from CustomerListForCalc;  
     ...
     Create Nonclustered index nci_CustomerListForCalc_CustomerNumber on #CustomerListForCalc (CustomerNumber) ;  
	    ...
	    insert into [InquiryReportCalculationPipelineLog]([Guid],Step,[State],[Description],CreationDate)
    values(NEWID(),'Fill Cte CustomerListForCalc','done',N'استخراج لیست مشتریانی که باید برایشون محاسبه گزارش استعلام انجام داد',GetDate());
...
Create Nonclustered index nci_GetAllMandeTashilatRiali_MainCustomerId on #GetAllMandeTashilatRiali (MainCustomerId) ;  
	    ...
	    insert into [InquiryReportCalculationPipelineLog]([Guid],Step,[State],[Description],CreationDate)
    values(NEWID(),'Fill Cte GetAllMandeTashilatRiali','done',N'  ترنسفر استخراج مبلغ تسهیلات به تفکیک طبقه و مشتری',GetDate())
...
insert   
 ...
 src.[LoanDoubtfullAmountEq]  
 	    ...
 	    insert into [InquiryReportCalculationPipelineLog]([Guid],Step,[State],[Description],CreationDate)
    values(NEWID(),'Fill Cte GetAllMandeTashilatRiali-Merge','done',N'  مرج استخراج مبلغ تسهیلات به تفکیک طبقه و مشتری',GetDate());
...
GROUP BY a.MainCustomerNumber,a.MainCustomerId
 ...
 Create Nonclustered index nci_DelyaedPenaltyAmount_MainCustomerId on #DelyaedPenaltyAmount (MainCustomerId) ;  
	...
	insert into [InquiryReportCalculationPipelineLog]([Guid],Step,[State],[Description],CreationDate)
    values(NEWID(),'Fill Cte DelyaedPenaltyAmount','done',N'  ترنسفر خسارت تاخیر پرونده های تسهیلات ریالی',GetDate())
 ...
 from #DelyaedPenaltyAmount t1 join ETL_TMP_InquiryReportData t2 on t1.MainCustomerId=t2.CustomerId;  
  	...
  	insert into [InquiryReportCalculationPipelineLog]([Guid],Step,[State],[Description],CreationDate)
    values(NEWID(),'Fill Cte DelyaedPenaltyAmount-Merge','done',N' ETL_TMP_InquiryReportData  مرج-بروزرسانی خسارت تاخیر پرونده های تسهیلات ریالی',GetDate());
 -------------------------------------------------------------------------------------------------  
...
GROUP BY MainCustomerNumber,MainCustomerId  
 ...
 from LoanSandoghToseeMeliAmountEq t1 join ETL_TMP_InquiryReportData t2 on t1.MainCustomerId=t2.CustomerId;  
   	...
   	insert into [InquiryReportCalculationPipelineLog]([Guid],Step,[State],[Description],CreationDate)
    values(NEWID(),'Fill Cte LoanSandoghToseeMeliAmountEq','done',N' ETL_TMP_InquiryReportData  ترنسفر مرج-بروزرسانی جمع مبلغ تسهیلات دریافتی از محل صندوق توسعه ملی به تفکیک مشتری',GetDate());
 -----------------------------------------------------------------------------------------------------------  
...
insert   
 ...
 set dest.InternalLCCurrentAmountEq=src.InternalLCCurrentAmountEq
   , dest.PrepaymentAmountCurrentLc=src.PrepaymentAmountCurrentLc;  
 	...
 	insert into [InquiryReportCalculationPipelineLog]([Guid],Step,[State],[Description],CreationDate)
    values(NEWID(),'Fill Cte InternlLCCurrentAmountEq','done',N'  ترنسفر  و مرج جمع مبلغ اعتبار اسنادی جاری به تفکیک مشتری',GetDate());
 ------------------------------------------------------------------------------------------------------------------  
...
insert   
 ...
 set dest.InternalLCDebtorAmountEq=src.InternalLCDebtorAmountEq 
  , dest.PrepaymentAmountDebtorLc=src.PrepaymentAmountDebtorLc;  
   	...
   	insert into [InquiryReportCalculationPipelineLog]([Guid],Step,[State],[Description],CreationDate)
    values(NEWID(),'Fill Cte InternalLCDebtorAmountEq','done',N'  ترنسفر  و مرج جمع مبلغ اعتبار اسنادی تبدیل به بدهی شده (بدهکاران) به تفکیک مشتری',GetDate());
...
group by c.MainCustomerNumber,c.MainCustomerId  
 ...
 Create Nonclustered index nci_InternalLCDoubtfullAmountEq_MainCustomerNumber on #InternalLCDoubtfullAmountEq (MainCustomerNumber) ;  
   	...
   	insert into [InquiryReportCalculationPipelineLog]([Guid],Step,[State],[Description],CreationDate)
    values(NEWID(),'Fill Cte InternalLCDoubtfullAmountEq','done',N'  ترنسفر  جمع مبلغ اعتبار اسنادی مشکوک الوصول به تفکیک مشتری',GetDate())
...
insert   
 ...
 from #InternalLCDoubtfullAmountEq t1 join ETL_TMP_InquiryReportData t2 on t1.MainCustomerId=t2.CustomerId;  
    	...
    	insert into [InquiryReportCalculationPipelineLog]([Guid],Step,[State],[Description],CreationDate)
    values(NEWID(),'Fill Cte InternalLCDoubtfullAmountEq-Merge','done',N'  مرج و بروزرسانی  جمع مبلغ اعتبار اسنادی مشکوک الوصول به تفکیک مشتری',GetDate());
...
select * into #GuaranteeCurrentAmountEq from GuaranteeCurrentAmountEq  
  ...
  Create Nonclustered index nci_GuaranteeCurrentAmountEq_MainCustomerNumber on #GuaranteeCurrentAmountEq (MainCustomerNumber) ;  
     	...
     	insert into [InquiryReportCalculationPipelineLog]([Guid],Step,[State],[Description],CreationDate)
    values(NEWID(),'Fill Cte GuaranteeCurrentAmountEq','done',N'  ترنسفر  جمع مبلغ ضمانت نامه های جاری به تفکیک مشتری',GetDate())
...
insert   
 ...
 dest.PrepaymentAmountCurrentGa=src.PrepaymentAmountCurrentGa;
       	...
       	insert into [InquiryReportCalculationPipelineLog]([Guid],Step,[State],[Description],CreationDate)
    values(NEWID(),'Fill Cte GuaranteeCurrentAmountEq-Merge','done',N'  مرج  جمع مبلغ ضمانت نامه های جاری به تفکیک مشتری',GetDate());
...
select * into #GuaranteeDebtorAmountEq from GuaranteeDebtorAmountEq  
  Create Nonclustered index nci_GuaranteeDebtorAmountEq_MainCustomerNumber on #GuaranteeDebtorAmountEq (MainCustomerNumber) ;  
    ...
    insert into [InquiryReportCalculationPipelineLog]([Guid],Step,[State],[Description],CreationDate)
    values(NEWID(),'Fill Cte GuaranteeDebtorAmountEq','done',N'  ترنسفر  جمع مبلغ ضمانت نامه های تبدیل بدهی شده به تفکیک مشتری',GetDate())
...
insert   
 ...
 set dest.GuaranteeDebtorAmountEq=src.GuaranteeDebtorAmountEq,
     dest.PrepaymentAmountDebtorGa=src.PrepaymentAmountDebtorGa;
      ...
      insert into [InquiryReportCalculationPipelineLog]([Guid],Step,[State],[Description],CreationDate)
      values(NEWID(),'Fill Cte GuaranteeDebtorAmountEq-Merge','done',N'  مرج  جمع مبلغ ضمانت نامه های تبدیل بدهی شده به تفکیک مشتری',GetDate());
...
Create Nonclustered index nci_GuaranteeDoubtfullAmountEq_MainCustomerNumber on #GuaranteeDoubtfullAmountEq (MainCustomerNumber) ;  
      ...
      insert into [InquiryReportCalculationPipelineLog]([Guid],Step,[State],[Description],CreationDate)
      values(NEWID(),'Fill Cte GuaranteeDoubtfullAmountEq','done',N'  ترنسفر  جمع مبلغ ضمانت نامه های مشکوک الوصول به تفکیک مشتری ',GetDate())
...
insert   
 ...
 dest.PrepaymentAmountMashkookGa=src.PrepaymentAmountMashkookGa;
      ...
      insert into [InquiryReportCalculationPipelineLog]([Guid],Step,[State],[Description],CreationDate)
      values(NEWID(),'Fill Cte GuaranteeDoubtfullAmountEq-Merge','done',N'  مرج  جمع مبلغ ضمانت نامه های مشکوک الوصول به تفکیک مشتری ',GetDate());
...
Create Nonclustered index nci_InDirectCommitmentsAmountEq_CustomerId on #InDirectCommitmentsAmountEq (CustomerId) ;  
        ...
        insert into [InquiryReportCalculationPipelineLog]([Guid],Step,[State],[Description],CreationDate)
      values(NEWID(),'Fill Cte InDirectCommitmentsAmountEq','done',N' ترنسفر  تعهدات غیر مستقیم ',GetDate())
...
insert   
 ...
 set dest.InDirectCommitmentsAmountEq=src.InDirectCommitmentsAmountEq;  
        ...
        insert into [InquiryReportCalculationPipelineLog]([Guid],Step,[State],[Description],CreationDate)
      values(NEWID(),'Fill Cte InDirectCommitmentsAmountEq-Merge','done',N' مرج  تعهدات غیر مستقیم ',GetDate());
...
Create Nonclustered index nci_TotalAssuranceAmountEq_CustomerNumber on #TotalAssuranceAmountEq (CustomerNumber) ;  
        ...
        insert into [InquiryReportCalculationPipelineLog]([Guid],Step,[State],[Description],CreationDate)
      values(NEWID(),'Fill Cte TotalAssuranceAmountEq','done',N' ترنسفر  جمع مبلغ وثایق هر مشتری ',GetDate())
...
insert   
 ...
 set dest.TotalAssuranceAmountEq=src.TotalAssuranceAmountEq;  
      ...
      insert into [InquiryReportCalculationPipelineLog]([Guid],Step,[State],[Description],CreationDate)
      values(NEWID(),'Fill Cte TotalAssuranceAmountEq-Merge','done',N' مرج  جمع مبلغ وثایق هر مشتری ',GetDate());
...
where @CalculateDate>=EffectiveDate and @CalculateDate<isnull(cast(expirationDate as date),getdate())
  ...
  [ETL_TMP_InquiryReportData] t1 join StockCostAmount t2 on t1.CustomerId=t2.CustomerId
   --where t2.UploadDate=(select cast(max(UploadDate) as date) from BankInvestment where UploadDate <= @CalculateDate)
      ...
      insert into [InquiryReportCalculationPipelineLog]([Guid],Step,[State],[Description],CreationDate)
      values(NEWID(),'Fill Cte StockCostAmount','done',N' ترنسفر و مرج  قیمت تمام شده سهام ',GetDate())
...
insert into [InquiryReportCalculationPipelineLog]([Guid],Step,[State],[Description],CreationDate)
values(NEWID(),'Update calculateDate  in ETL_TMP_InquiryReportData ','done',N' ETL_TMP_InquiryReportData بروز رسانی فیلد تاریخ میلادی و شمسی در جدول تمپ ',GetDate())
...
from  ETL_TMP_InquiryReportData
      ...
      insert into [InquiryReportCalculationPipelineLog]([Guid],Step,[State],[Description],CreationDate)
      values(NEWID(),'insert InquiryReportData','done',N'درج در جدول InquiryReportData ',GetDate())
  ...
  and PackageName=N'InquiryReportData'
        ...
        insert into [InquiryReportCalculationPipelineLog]([Guid],Step,[State],[Description],CreationDate)
      values(NEWID(),'Delete CalculationTime  for package InquiryReportData','done',N'حذف رکورد Calculationtime for today  ',GetDate())
...
null,
 @CalculateDate as Calculatetime,
  N'InquiryReportData' as PackageName
         ...
         insert into [InquiryReportCalculationPipelineLog]([Guid],Step,[State],[Description],CreationDate)
      values(NEWID(),'insert CalculationTime  for package InquiryReportData','done',N'درج رکورد Calculationtime for today  ',GetDate())
  ...
  EXEC [SP_CPReports_FillFlatTables] @CalculateDate
       ...
       insert into [InquiryReportCalculationPipelineLog]([Guid],Step,[State],[Description],CreationDate)
      values(NEWID(),'[SP_CPReports_FillFlatTables]','done',N'اجرای [SP_CPReports_FillFlatTables] ',GetDate())


	         insert into [InquiryReportCalculationPipelineLog]([Guid],Step,[State],[Description],CreationDate)
      values(NEWID(),'Last Step Calculation is Finished','done',N'اتمام محاسبات',GetDate())
